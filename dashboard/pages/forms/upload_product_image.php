<?php
  ...
  $dblist_string="SELECT * FROM `item` WHERE `haveimage` = 0 ORDER BY itemname";
  $dbdata = mysqli_query($con,$dblist_string);

  $dblist_string1="SELECT * FROM `item` WHERE `haveimage` = 1 ORDER BY itemname";
...
<!-- </a> -->
    ...
    <div class="box-body">
        <table id="example1" class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>SL</th> <th>Code</th>  <th> Name </th> <th> Image </th> <th> Action </th>
                </tr>
            </thead>
            <tbody>
                    <?php
                    ...
                    if ($dblist>0) {
                      ...
                      while($row = $dbdata->fetch_assoc()) {
                        ...
                        if ($status==0) {
                          ...
                          ?>
                              <tr>
                                  <td><?php echo $i;?></td><td><?php echo $row["itemcode"];?></td><td><?php echo $row["itemname"];?></td>
                              ...
                              if ($dblistt>0) {
                                 while($row = $dbdata1->fetch_assoc()) {
                                  ...
                                  $itemname=$row['itemname'];
                                    ?>
                                      <option value="<?php echo $id;?>"><?php echo $itemname.'('.$itemcode.')';?></option>
                                    <?php
                                 }
                               }
                            ?>
      ...
      if (file_exists($file_path)) {
            ...
            if( $moved ) {
                //echo "Successfully uploaded"; 
                 $insert_string="UPDATE `item` SET `image` = '$file_name',`haveimage` = 0,`status` = 0 WHERE `item`.`id` = $shid";             
                 $insertresult = mysqli_query($con,$insert_string);
                  if ($insertresult==1){
                      echo '<script>window.alert("Succefully added")</script>';
                      echo '<script>window.location.href = "upload_product_image.php";</script>';
                    }else{
